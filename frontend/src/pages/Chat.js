      ...
      {descendingOrderMessages.map((message,_index)=>{
        ...
        <div key={_index}>
              {message.id!==id && <div className='flex flex-row items-center justify-start mt-4'>
                <img src={message.img ? message.img : maleUser} className='h-8 rounded-full' />
                <div className='ml-2'>
                  <h2 className='text-xs opacity-70'>{message.name}</h2>
                  <div className=' text-sm md:text-base pt-1 pb-1 pl-2 pr-2 bg-[#fd2f6e] rounded text-white break-words max-w-[10rem] sm:max-w-[15rem] lg:max-w-sm'>{message.message}</div>
                </div>
            </div>}
            {message.id===id && <div className='flex flex-row items-center justify-end mt-4'>
                ...
                <div className='mr-2'>
                  ...
                  <h4 className=' text-sm md:text-base text-left pt-1 pb-1 pl-2 pr-2 bg-[#FFD9C0] rounded break-words max-w-[10rem] sm:max-w-[15rem] lg:max-w-sm'>{message.message}</h4>
                </div>
                <img src={message.img ? message.img : maleUser} className='h-8 rounded-full' />
            </div>}
          </div>
