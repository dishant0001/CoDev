      ...
      <div className='flex flex-col md:flex-row justify-center items-center'>
        ...
        <div className=' md:ml-8 lg:ml-10'>
          ...
          <p className='text-sm lg:text-base ml-3 md:ml-0'>
            {user.show_dob && <span><i className="fa-solid fa-cake-candles"></i> {user.dob_day}-{user.dob_month}-{user.dob_year}</span>}
            {user.show_gender && user.gender==="man" && <span><i className="fa-solid fa-mars ml-4"></i> Male</span>}
            {user.show_gender && user.gender==="woman" && <span><i className="fa-solid fa-venus ml-4"></i> Female</span>}
          </p>
          <p className='ml-3 mr-3 md:ml-0 md:mr-0 mt-2 lg:mt-3 text-sm lg:text-base'>{user.about}</p>      
        </div>
      </div>
